...
(`datos.gov.co`, dataset p6dx-8zbt) tomada el 2026-07-21 (registros 2026 que
matchean la keyword "tecnolog"). Nada de payloads inventados: los casos borde
(campo ausente, sin URL) se ejercitan removiendo campos de copias de registros
reales, no fabricando datos.
"""
...
def test_mapeo_registro_real(conn, registros, httpx_mock) -> None:
    httpx_mock.add_response(json=registros)  # una sola página (5 < PAGE_SIZE)
    ...
    assert item.id_externo == "CO1.REQ.9451056"
    assert item.titulo == "ADNERIS APARICIO FLOREZ"
    assert item.descripcion and "TECNOLOGÍAS DE LA INFORMACIÓN" in item.descripcion
    assert item.entidad == "SECRETARIA DISTRITAL DE GOBIERNO 1"
    assert item.tipo == "licitacion"
    assert item.estado_fuente == "Seleccionado"
    assert item.modalidad == "Contratación directa"
    assert item.monto == Decimal("118096000")
    assert item.moneda == "COP"
    assert item.departamento == "Distrito Capital de Bogotá"
    assert item.ciudad == "Bogotá"
    assert item.pais == "Colombia"
    assert item.fecha_publicacion == datetime(2026, 1, 2, tzinfo=timezone.utc)
    assert item.fecha_publicacion.tzinfo is not None  # UTC-aware
    # El dataset no expone deadline ni requisitos -> None (no se inventan).
    ...
    assert item.url_original == (
        ...
        "OpportunityDetail/Index?noticeUID=CO1.NTC.9369835"
    )
...
def test_paginacion_dos_paginas(conn, registros, httpx_mock) -> None:
    conn.PAGE_SIZE = 3  # con 5 registros reales -> pág1=3 (llena) + pág2=2 (parcial)
    pagina1 = registros[:3]
    pagina2 = registros[3:]
    ...
    assert requests[1].url.params.get("$offset") == "3"
